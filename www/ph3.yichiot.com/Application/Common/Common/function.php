...
function equip_upload($supply){
    //实例化上传类
    $upload = new \Think\Upload();
    //类外改属性
     $upload->maxSize   =     500*1024*1024 ;// 设置附件上传大小  word，excel，pdf，jpg，txt  
    $upload->exts      =     array('jpg','doc','xls','txt','pdf');// 设置附件上传类型    
    $upload->savePath  =      './Public/album/'; // 设置附件上传目录
    //类外调方法
    $info = $upload->uploadOne($supply);
    //pp($info);
    if($info)
    {
        //得到路径入库
...
function app_upload_image($path,$maxSize=52428800){
    ...
    $config=array(
        'rootPath'  =>'./',         //文件上传保存的根路径
        'savePath'  =>'./'.$path.'/',   
        'exts'      => array('jpg', 'gif', 'png', 'jpeg','bmp','txt'),
        'maxSize'   => $maxSize,
        'autoSub'   => true,
        );
 ...
 */
function post_upload($path='file',$maxSize='52428800'){
    ini_set('max_execution_time', '0');
    // 去除两边的/
    $path=trim($path,'/');
    // 添加Upload根目录
    $path=strtolower(substr($path, 0,6))==='Public/upload' ? ucfirst($path) : 'Public/Upload/'.$path;
...
function create_xls($data,$filename='simple.xls'){
      ...
      $filename=str_replace('.xls', '', $filename).'.xls';
       ...
       /* $phpexcel->getActiveSheet()->fromArray('1111');*/
        ...
        /*$phpexcel->getActiveSheet()->setCellValue('A1','111');*/



        // Field names 第一行 设备名 in the first row
        $phpexcel -> getActiveSheet() ->getColumnDimension(A)->setWidth(35);
      $col = 'B';
        foreach ($data['arr'] as $key=>$val)
        {
            $phpexcel -> getActiveSheet() -> setCellValue($col.'1', $key);
            $phpexcel -> getActiveSheet() ->getColumnDimension($col)->setWidth(35);
            $col++;
        } 
  
       //Field names日期 in the first col
      $row = 2;
        foreach ($data['list'] as $key => $value) {
                ...
                foreach ($value as $kk => $vv) {
                    if($data['style']=='temperature'){
                         $phpexcel -> getActiveSheet() -> setCellValueByColumnAndRow($col, $row,$vv['temperature'].'°C');

                    }elseif($data['style']=='humidity'){
                         $phpexcel -> getActiveSheet() -> setCellValueByColumnAndRow($col, $row,$vv['humidity'].'%RH');

                    }elseif($data['style']=='concentration'){
                        $phpexcel -> getActiveSheet() -> setCellValueByColumnAndRow($col, $row,$vv['concentration'].'ppm');
                    }else{
                         $phpexcel -> getActiveSheet() -> setCellValueByColumnAndRow($col, $row,$vv['co'].'ppm');
                    }
