...
function toggleFun(tab,content){
	var $tab =  $(tab);   
	//$tab.find("th").eq(0).addClass("actives");

	$tab.find("th").on("click",function(){
		var index = $(this).index();
		$(this).addClass("actives").siblings().removeClass("actives");
		$(content).children("table").eq(index).show().siblings().hide();


		//查询那里的温度
		$(".dw").text(dw[index]);  //alert($(".dw").text());
		$('.dwval').val(index);
...
toggleFun(".table_top",".table-wrap");

$(function () {
var dw = ['℃','%RH','ppm','ppm'];
	$(".con_middle .table_top th").each(function(){
		if($(this).hasClass("actives")){
			var index =$(this).index();
			$(".table-wrap table").eq(index).show().siblings().hide();
	...
	var flag = true;
	$('.titles li:eq(1)').click(function(){
		$(".one-wrap .one").toggle();
		if(flag){
			$(this).find("a").text("切换至报表");
			flag = false;
		}else{
			$(this).find("a").text("切换至曲线");
			flag = true;
		}
	...
	$(".add-per .save,.add-per .cancle-z").click(function(){
		$(".add-per").hide();
	})

	//菜单在当前页面效果
	if($(document.body).width() > 1024){
		$(".con_left").height($(window).height() - 120);
		$(".con_left ul>li a").each(function () {
	...
	if($(document.body).width() < 1024){
		...
		$("#up").height($(document.body).height())
	}
